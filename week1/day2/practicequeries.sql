...
SELECT department, COUNT(*) AS total_employees FROM Employees GROUP BY departmentHAVING COUNT(*) > 3;
2. Find departments where average salary is greater than 60000
SELECT department, AVG(salary) AS avg_salary FROM Employees GROUP BY departmentHAVING AVG(salary) > 60000;
3. Find cities having more than 2 employees
SELECT city, COUNT(*) AS total_employees FROM Employees GROUP BY cityHAVING COUNT(*) > 2;
4. Find departments where total salary is greater than 200000
SELECT department, SUM(salary) AS total_salary FROM Employees GROUP BY departmentHAVING SUM(salary) > 200000;
5. Find departments where maximum salary is above 90000
SELECT department, MAX(salary) AS max_salary FROM Employees GROUP BY departmentHAVING MAX(salary) > 90000;
